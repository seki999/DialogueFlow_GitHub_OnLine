...
- 解析：`subpoena` 是法律术语，指要求某人出庭作证或提交文件的正式传票。

</details>

### 第 2 题

`subpoena` 中的 `oe` 应该怎么读？
...
- 解析：这是 subpoena 发音的难点，拼写中的 oe 不按常规规则发音，而是发长元音 /iː/，重音也落在这个音节上。
...
- [ ] C. interview a witness
- [ ] D. dismiss a witness

<details>
<summary>提交选择后查看结果</summary>

- 正确答案：**B. subpoena a witness**
- 对错判断：选择 B 为正确；选择 A、C 或 D 为错误。
- 解析：`subpoena` 作及物动词时，可以直接接“证人”作为宾语，表示用传票传唤该证人出庭。
...
- [ ] A. He was subpoena to court.
- [ ] B. He was subpoenaed to court.
- [ ] C. He was subpoenaing to court.
- [ ] D. He is subpoena to court.

<details>
<summary>提交选择后查看结果</summary>

- 正确答案：**B. He was subpoenaed to court.**
- 对错判断：选择 B 为正确；选择 A、C 或 D 为错误。
- 解析：`subpoena` 作动词的被动语态需要用过去分词 `subpoenaed`，不能直接使用原形。

</details>

### 第 5 题

`subpoena` 和 `summons` 的主要区别是什么？

- [ ] A. 完全没有区别，可以随意互换
- [ ] B. subpoena 专指法律传票，summons 是更通用的传唤通知
- [ ] C. summons 只用于刑事案件，subpoena 只用于民事案件
- [ ] D. subpoena 是动词，summons 只能作名词

<details>
<summary>提交选择后查看结果</summary>

- 正确答案：**B. subpoena 专指法律传票，summons 是更通用的传唤通知**
- 对错判断：选择 B 为正确；选择 A、C 或 D 为错误。
- 解析：subpoena 专门指要求出庭作证或提交证据的法律传票；summons 含义更广，泛指正式的出庭通知，两者不完全等同。
